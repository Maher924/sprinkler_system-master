...
class UserID with ChangeNotifier {
  ...
  Map<String, dynamic> sprinklerStatus = {
    '0': true,
    '1': true,
    '2': true,
    '3': true,
  ...
  void push_sprinkler_data() async {
    db
        .collection("sprinklersID")
        .doc('${userID?.uid}')
        .set(sprinklerData)
        .onError((e, _) => print("Error writing document: $e"));
    db
        .collection("sprinklersStatus")
        .doc('${userID?.uid}')
        .set(sprinklerStatus)
        .onError((e, _) => print("Error writing document: $e"));
    notifyListeners();
  ...
  String calc_active() {
    int x = 0;
    for (int i = 0; i < 6; i++) {
      if (sprinklerStatus['$i'] == true) x++;
    }

    return x.toString();
  }

  String calc_deactive() {
    int x = 0;
    for (int i = 0; i < 6; i++) {
      if (sprinklerStatus['$i'] == false) x++;
    }
